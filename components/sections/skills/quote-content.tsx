...
type Props = {
  quote: string, autor: string, image: string, rol: string
}
const QuoteContent = ({ autor, rol, image, quote }: Props) => {
  return (
    <div className='flex flex-col w-full p-8 gap-4'>
      <div className='px-4 py-2 relative'>
        <p className='text-lg text-center text-color italic font-md'>{quote}</p>
      ...
      <div className='flex flex-col justify-center items-center w-full'>
        ...
        <p className='text-color-secondary font-light text-sm'>{rol}</p>
      </div>
    </div>
  )
}
