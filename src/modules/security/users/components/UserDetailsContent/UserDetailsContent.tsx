...
const UserDetailsContent = () => {
  ...
  return (
    <Box pt={1}>
      <TabsHeader>
        <RouterTab tabs={accountTabs} prefix={`/security/users/${user?._id as string}`} translationNs={'account'} />
      </TabsHeader>
      <Box>
        <RouteLoader routes={accountRoutes} notfoundRedirect={`/security/users/${user?._id as string}/general`} />
      </Box>
    </Box>
  );
};
