...
export const useFindExpressDeliveryPlaces = (type?: LOCATION_TYPE, country?: string | null, state?: string | null) => {
  ...
  const { fetch, queryKey } = useTableRequest(ExpressDeliveryPlacesService.search, filters);

  return useQuery([EXPRESS_DELIVERIES_LIST_KEY, queryKey, state, type], fetch);
};
