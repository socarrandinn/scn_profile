...
class HomeDeliveryPlacesService extends EntityApiService<IHomeDelivery> {
  ...
  updatePriceConfig (params: any) {
    return this.handleResponse(
      ApiClientService.patch(this.getPath(`/${params?._id as string}/price-config`), { ...params, enabled: !params?._id }),
    );
  }

  saveOrUpdateCustom = (params?: any, config?: RequestConfig) => {
    if (params?._id) {
      return this.handleResponse(ApiClientService.patch(this.getPath(`/${params?._id as string}/price-config`), params, config));
    }
