...
class ProductService extends EntityApiService<IProduct> {
  ...
  changeVisibilityManyStock = (
    ...
    if (ids) {
      return ApiClientService.patch(this.getPath(`/${productId}/stock/bulk/visibility`), {
        ids,
        visible,
      });
    }
  ...
  searchWarehouseStock = (productId: string, params?: any, config?: any): any => {
    return this.handleResponse(ApiClientService.post(this.getPath(`/${productId}/stock/search`), {}, config));
  };
}
