...
const WarehouseSupplierForm = ({ control, watch, isLoading, onSubmit }: WarehouseSupplierFormProps) => {
  const { t } = useTranslation('warehouse');
  const { warehouse } = useWatch({ control });

  return (
    <Box mt={2}>
      <Form onSubmit={onSubmit} control={control} isLoading={isLoading} id={'warehouse-supplier-form'} watch={watch}>
