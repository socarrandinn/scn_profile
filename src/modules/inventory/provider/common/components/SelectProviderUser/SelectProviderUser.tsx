...
import { useFetchProviderUser } from '../../hooks/useFetchProviderUser';

type SelectProviderUserProps = {
  name: string;
  placeholder?: string;
  helperText?: string;
...
const SelectProviderUser = ({
  ...
  fetchOption,
  helperText,
}: SelectProviderUserProps) => {
  const { fetch } = useFetchProviderUser();
  return (
...
export default memo(SelectProviderUser);
