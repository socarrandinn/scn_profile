export { default as SupplierGeneralPage } from './SupplierGeneralPage';
export { default as SupplierInventoryPage } from './SupplierInventoryPage';
export { default as SupplierProductPage } from './SupplierProductPage';
export { default as SupplierProductsPage } from './SupplierProductsPage';
export { default as SupplierReportSalePage } from './SupplierReportSalePage';
export { default as SupplierHistoryChangePage } from './SupplierHistoryChangePage';

// tabs users
export { default as SupplierUsersPage } from './SupplierUsersPage';
export { default as SupplierUserInviteList } from './SupplierUserInviteList';
