...
export interface IAdvertisingBoxProps {
  item: IAdvertisement;
}
...
const AdvertisingBox = ({ item }: IAdvertisingBoxProps) => {
  ...
  return (
        <Card>
            <TitleStyled alignItems='center' justifyContent='space-between' bg={AdvertisementColorConstants[item.type]}
                         p={2}>
                ...
                <FlexBox alignItems='center' justifyContent='flex-end' width='100%' sx={{ position: 'absolute' }}>
                    {getImg}
                </FlexBox>
            </TitleStyled>
